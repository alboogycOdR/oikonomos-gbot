...
- **NEVER run `pnpm test` / `pnpm --filter <pkg> test` / `vitest` directly.** The `DATABASE_URL` in your environment by default points at the real, live production database — a direct test invocation writes real fixture rows there and, for any package with a real-provider test (chatRunDriver, control-api's real-Postgres suites), can incur real, unintended paid spend. This has already happened twice this session (once flagged in a review, once caught live afterward — real fixture pollution and real Claude/Gemini spend landed in production both times). **Always** run `powershell -ExecutionPolicy Bypass -File scripts\test-isolated.ps1` (optionally `-Filter <pkg>`) instead — it redirects to the isolated `oikonomos_test` database for the duration and restores everything after. If you need a fast, no-DB signal while iterating (typecheck, lint, a pure-function test with no `DatabaseOptions`), that's fine directly; anything that touches Postgres or a real provider is not.
