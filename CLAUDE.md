...
- **Builder roster (2026-09-24, Alister): only CX9 and S5 build.** GB (Grok Build) is retired and CX is inactive; the roster line in the DEVDEPARTMENT section below is stale and loses to this bullet. Give CX9 proportionally more tasks than S5 (about two to one). CX9 is also the only non-Claude builder, so it takes protected-path work that needs a different-model reviewer.
