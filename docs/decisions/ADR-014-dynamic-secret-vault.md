...
## Amendment 2026-09-14 — connector-minter consumption deferred; scope split from TASK-250

TASK-250 (filed from TASK-240's config-check finding that `OIK_SECRET_VAULT_KEY` is unset everywhere because nothing calls the vault in production) investigated both halves of this ADR's §1 promise — "the only legitimate callers are host-side connector session minters and the future OIK-045a injector" — against the live code, not the ADR's description of it. Findings, verified by CX9 and independently plausible against everything else this project has found about the connector layer:

**The write half is genuinely wirable now.** `services/control-api/src/index.ts:start()` never passes `secretRequests` to `buildApp`, so `POST /secret-requests/:id/fulfil` and `/decline` both 501 (`app.ts:2103-2105`, `:2129-2131`); `packages/db/src/secretRequests.ts:88-99`'s fulfil path only ever updates `secret_requests.status`/`secret_ref`/`fulfilled_at` — it has never called `secretVault.store`, so no `secret_values` row and no `secret_vault.write` audit event have ever been produced by a real fulfilment, only by this package's own tests. This half — control-api composition, the fulfilment handler actually storing through the vault, and the write-side audit event — is in scope for the resolver's own package boundaries and does not touch the connector layer at all. It remains TASK-250's job.

**The read half — a connector minter resolving a vault-minted `secret://<ref>` — is not a small addition, and is deferred, not merely unwired.** `services/worker/src/chatRunDriver.ts:211-247` constructs one shared, role-agnostic minter pool per connector (Gmail, Calendar, Drive) at worker composition time; each minter is built once against `packages/connectors/src/mcp/envSecretResolver.ts`, which resolves only the fixed, manifest-declared `secret://mcp/.../…` refs backed by static `OIK_SECRET_*` environment variables (`packages/connectors/manifests/{gmail,google-calendar,google-drive}.yaml` name only these). There is no seam in that composition for a resolver bound to one role and one run to be substituted per-request, and there cannot safely be one without changing the pool from shared-and-cached to scoped-and-per-request — a connector-composition change, not a vault change. Building it properly is real, separately-scoped work: TASK-253.

**Decision:** ADR-014's §1 promise is honoured in two phases rather than one. Phase 1 (TASK-250, this session) makes the vault's write path and its audit trail real and load-bearing — a human-fulfilled secret is genuinely encrypted, stored, and audited, exactly as designed, satisfying every part of this ADR except the connector read side. Phase 2 (TASK-253, filed, not yet scheduled) designs and builds the per-role/run-scoped minter resolution needed for a connector to actually consume a dynamically-vaulted secret; until it lands, `request_secret`'s only real effect is that a human-supplied value is safely at rest and audited — it cannot yet be handed to a connector session. This is a materially narrower promise than §1's original single-paragraph framing implied, and is recorded here rather than silently accepted.

**Consequence for TASK-250's acceptance bar:** its AC2's "either/or" is resolved to the second branch — this dated note — for the connector-consumption half specifically; the write-path-plus-audit half stays a hard requirement, not an alternative to be traded away.
