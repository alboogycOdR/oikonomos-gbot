...
## Multi-agent coordination (added 2026-08-26, ORCH — group-chat source read + research2 companion pack)

Scope this study originally missed: the three dispatched agents covered permission broker, provider routing, and CI/security, but nobody read the group-chat/messaging layer. Filled in two ways: (1) direct source reads of `source/host/groups/group-chat.ts`, `source/host/extensions/transcript/group-chat-orchestrator.ts`, `source/host/extensions/transcript/agent-to-agent-messaging.ts`, `source/host/agents/agent-messaging.ts`; (2) a third-party research pack at `research2/Grok-Bot-Agent-Blueprint/` (pinned to the same repo, commit `a9f633e0`, dated 2026-08-25) — its own claims were spot-verified against source before trusting them (all matched exactly, including the constants below and the priority/user-lane guard).

Directly answers "how do agents communicate / assign roles / decide who works first" for this codebase:

**(a) Communication — two distinct primitives, not one.**
- *Group rooms*: agents post to a shared transcript (the room is itself a background agent session with `memberIds`); no direct RPC between agents, purely history-mediated, like a channel.
- *1:1 async messaging (`SendToAgent`)*: fire-and-forget — sender gets an immediate delivery acknowledgement (`"Sent to <name>."`), never a reply in the same turn; the recipient wakes on a **later hidden turn** cued `[agent]`; a reply is a new message, not a return value. This is the piece the original study missed entirely — it's the actual mailroom primitive, separate from rooms. Verified the literal system prompt shown to the model at `source/host/agents/agent-messaging.ts:40-47`, richer than the research pack's summary: explicit anti-fan-out judgment ("propose it first... unless the user explicitly told you to contact those agents"), a private-relay non-disclosure rule (never relay the user's unfiltered words/venting verbatim), a no-ping-pong rule (don't reply just to acknowledge), and — notable for a permission model — **only the user can delete an agent, never another agent or itself**.

**(b) Role assignment — thinner than the marketing material implies, confirmed twice now.** `GroupMember` is `{id, name, description}` — free text, no structured role/skill/capability/tier field. A named "teammate" is a differently-configured single-agent session (own system prompt, own tools), not membership in a role registry. This is the clearest place OIKONOMOS is already ahead by design: our `role_id` + `role_grants` + per-capability `risk_tier` schema is a real structured role system this codebase doesn't have.

**(c) Work ordering — two independent mechanisms layered.**
- *Within a room*: bounded round-robin, not free-for-all. Verified constants at `source/host/groups/group-chat.ts:1` and `source/shared/agents/agents.ts:53`: `GROUP_MAX_MEMBERS=6`, `GROUP_MAX_ROUNDS=3`, `GROUP_MAX_MEMBER_TURNS=10`, `GROUP_MAX_MESSAGES_PER_TURN=2`, `SHARED_ROOM_HISTORY_LIMIT=24`. Each round, `resolveResponders()` narrows to @-mentioned members (default: everyone); `orderRoundSpeakers()` rotates the starting speaker (`round % memberCount` offset); a literal `"(pass)"` reply is dropped, not posted; a round producing zero messages ends the room; an epoch-cancellation check aborts the whole in-flight round-robin on a new human message.
- *Across the whole agent, priority vs. lane*: `source/host/extensions/transcript/agent-to-agent-messaging.ts:125-140` — a priority 1:1 message calls `steerRecipientForPriorityPeer`, which checks `scheduler.getActiveLane(agentId) === "user"` and **returns without interrupting** if true; otherwise it interrupts the recipient's active non-user (group-member or background) run and records why (`reason: "superseded by a priority agent message"`), tracked so the caller can tell a preempted run from a normally-finished one. Priority is 1:1-only — explicitly ignored for group posts (own message: *"priority is 1:1 only — this post did not interrupt members"*). Net authority order: active user turn > priority 1:1 peer message > normal peer message > background/automation.

**Corrections to the marketing-video framing** (research2's own "what the video simplified" table, source-verified): "each agent has its own computer" is wrong — agents share one sandbox, each gets a private desktop *window* from a finite pool via an owner-token map (`source/host/box/shared-desktop-sand-box.ts`), so a private screen does not imply a private filesystem. "It learns after a few approvals" is wrong — standing modes are explicit (`always/ask/never`, default `ask`, admin-clampable ceiling — the same rank-clamp shape as our own TASK-078), approval resolutions are four-way (`allow once/deny/always/never`), allow-once is scoped and retired at scope-end or next turn, denials are remembered for the current direction so a rejected action can't be immediately re-asked, and **repetition never silently promotes to a standing grant**. "Import your skills from Codex/Claude Code" is wrong — the real importer only discovers `CLAUDE.md`/`AGENTS.md`/`.claude/CLAUDE.md`/`.cursor/rules/*`, not whole skill folders.

**New patterns worth naming for E10** (multi-agent, currently pending in WBS — no tasks queued yet, this is groundwork not a decompose):
- The async-send-then-later-wake pattern (with delivery-ack ≠ reply, durable pending-wake state, retry-without-duplicate-work) is a cleaner mailroom primitive than anything in our current spec for agent-to-agent traffic, and composes well with our own typed-handoff design (OIK-102) rather than competing with it — theirs is unstructured text, ours is typed objects with fact references; the wake/ack mechanics are reusable regardless of payload shape.
- The priority-interrupt-with-protected-user-lane pattern is directly reusable for OIKONOMOS's own run scheduler once multiple concurrent routines exist (maps onto **TASK-076**'s lane work already queued — extend it with this same guarded-interrupt shape rather than lane-priority alone).
- The room's hard bounds (member cap, round cap, total-message cap, per-turn cap, history window) plus "zero-new-messages round ends it" are a complete, copyable anti-runaway-cost design if/when OIKONOMOS ever does a live multi-agent deliberation room (not currently specced; note for whoever picks up E10 design).
- Four-way approval resolution (`allow once/deny/always/never`) with denial memory and a non-retroactive admin ceiling is close to a superset of our own approvals model — worth a read against `packages/approvals` when TASK-073 (refusal memory) is picked up, since this is independent confirmation of the same design from the same codebase's *other* subsystem.
