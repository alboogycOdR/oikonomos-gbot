...
2. ~~**Draft-only by construction.** Its tool list contains no send tool whatsoever~~ — **CORRECTED 2026-09-01 (ORCH), proven wrong by live enumeration.** A real `tools/list` against this exact server (via an interactive `/mcp` session) returned `send_message`, `reply`, and `forward` alongside the read/draft/label tools this bullet originally listed, plus mutating label/trash/spam tools (`label_message`, `create_label`, `delete_label`, `trash_message`, `mark_message_spam`, thread-level equivalents, etc.). **The "send is impossible" property is NOT enforced by the server surface.** Only the missing `gmail.send` OAuth scope stands at that layer (assuming the token truly can't call `send_message` without it — not yet independently verified). OIKONOMOS's own allowlist/broker defence-in-depth (Handover §4.2 unregistered⇒deny; `email.send enabled:false`) is therefore the primary control here, not a backup — TASK-054 must explicitly prove `send_message`/`reply`/`forward` land unmapped-and-denied by name, not merely that `email.send` isn't in the capability map.
