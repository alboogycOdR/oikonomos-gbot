...
### 18. Resume-path builder prompt points at the builder's own last Progress_Note, not at ORCH's Review_Findings — a builder can miss a fresh review round entirely

**Severity: medium — a builder can correctly conclude "nothing new" while a new review round sits unread, and its own control block then locks the task out of re-dispatch. Found live 2026-08-22.** After ORCH wrote a fourth round of review findings to a task's `Review_Findings` (committed and pushed before the next dispatch), S5 was re-dispatched to act on it. Its resume-path instructions direct it to "re-read its Owned_Paths files and the last Progress_Note to find the exact stopping point" — Progress_Notes are the builder's own heartbeat, and S5 had not written one since the prior round, so by that check nothing looked new. S5 concluded no work was pending and emitted `status: needs_review` with an explicit "no new review findings since" note — despite the actual newest findings entry sitting one field over, already on disk.

**The compounding part:** `needs_review` is a real status value the SV drain applies literally. The very next dispatch attempt then read the task as `needs_review` — not `in_progress`/`claimed` — and reported **"no eligible task for S5, skipping launch"**, because a `needs_review` task isn't in either bucket a resume-first dispatch checks. A single wrong status word from a no-op session was enough to make the task briefly unreachable by the normal claim/resume path, requiring an ORCH manual status correction to unstick.

**Root cause:** the builder prompt's "what's new for me" check is scoped to `Progress_Notes` (self-authored) and never told to diff against `Review_Findings` (ORCH-authored) for content newer than the builder's last note. A builder with nothing of its own to report has no prompted reason to look at the field where the actual new instruction lives.

**Recovery performed:** ORCH reset `Status` to `in_progress` with a note explaining the wrong transition, re-pushed, then re-dispatched successfully.

**Recommend, in order of preference:**
1. **Prompt fix (cheapest):** the resume-path instructions should direct the builder to compare its own last `Progress_Notes` timestamp against `Review_Findings`' newest timestamp, and treat any `Review_Findings` entry newer than its last note as unread, regardless of whether it wrote a note since.
2. **A no-op session should never be allowed to emit `needs_review`.** If a builder does zero work and zero file changes, its own control block's status should be constrained to `in_progress` (or `blocked`) — `needs_review` should require at least one commit since the task's last review round, checkable mechanically (the same "verify a claim against a fact" principle as finding #17).
3. **Dispatch should treat `needs_review` as claimable-for-resume too** when the eligible-task search comes up empty for a unit that owns an in-flight `needs_review` task — or at minimum surface a distinct warning ("task X is needs_review but was assigned no reviewer action - orphaned?") rather than the generic "no eligible task" message, so the operator doesn't have to reconstruct the causal chain by hand.
...
Eight source-confirmed pack-code defects (items 1–8), two test-scaffolding defects found live in a resync (items 11–12, both traced to source before reporting so neither is a false positive), three live defects in the dispatch/control path (item 16, a claim-commit clobber on 2026-08-18; item 17, a fabricated needs_review from the extractor scraping its own prompt example on 2026-08-20; item 18, a no-op resume session emitting needs_review and locking the task out of re-dispatch on 2026-08-22), one confirmed design gap that cost real diagnostic time across three rounds (item 9), one dormant/unwired feature worth surfacing rather than leaving silent (item 10), and two protocol-level lessons worth folding into onboarding guidance. None of these are OIKONOMOS-specific — all are properties of the DEVDEPARTMENT scripts and templates as shipped, reproducible on any project using multi-builder dispatch with worktrees on Windows/PowerShell.
