...
pg_dump --format=custom --no-owner --no-privileges --file "$archive_dir/postgres.dump" "$DATABASE_URL"
pg_dump --data-only --inserts --no-owner --no-privileges "$DATABASE_URL" | sed '/^\\restrict /d; /^\\unrestrict /d' | sha256sum | awk '{print $1}' > "$archive_dir/postgres-data.sha256"
docker run --rm --volume "$EVIDENCE_VOLUME:/source:ro" --volume "$archive_dir:/backup" alpine:3.20 tar -C /source -czf /backup/evidence.tar.gz .
