...
- `OIKONOMOS_TEMPLATES_v1.0.md` — 2026-09-12 (Fable) installable bot recipes: canonical manifest, refuse-on-secret export/install, install never grants, internal-only v1. Decisions in ADR-018. Not yet decomposed.
- `OIKONOMOS_PROJECT_WORKSPACE_v1.0.md` — 2026-09-12 (Fable) Project entity (thread + board + artifact register + decision log) and the manager bot as a Project role with governed `project.*` capabilities; bot creation/grants declared-disabled. Decisions in ADR-019. Sequenced after Wave Workspace-1; not yet decomposed.
