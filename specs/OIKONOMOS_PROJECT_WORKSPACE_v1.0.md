...
> **Changelog:** v1.2 (2026-09-19, ORCH, owner decision) -- the manager creates and retires bots through the existing `workspace.create_bot` / `workspace.retire_bot` tools (approval-gated), not through a `project.create_role` capability; `project.create_role` is no longer declared (§0, §7.2, §7.3, §11). See `docs/decisions/ADR-019-project-entity-and-manager-role.md` Amendment 2026-09-19.
>
...
| Does the manager create bots or grants in v1? | **No.** v1 manager routes work among roles a human already provisioned and added to the roster. `project.create_role` and `project.request_grant` are **declared as capabilities but disabled** (`enabled: false`, like `email.send` in `gmail.yaml`), at T4 and T3 respectively, so the approval boundary exists in the tier map before anything can use it. Enabling them is a later ADR plus a `require_approval_rules` row. (ADR-019 §4) **Amended 2026-09-19 (owner decision):** the manager creates and retires bots through the existing, approval-gated `workspace.create_bot` / `workspace.retire_bot` tools; `project.create_role` is not declared; `project.request_grant` stays declared and disabled. (ADR-019 Amendment 2026-09-19) |
...
- A liveness test through the real broker→MCP composition invokes every tool the *project* MCP server mounts and proves `roles` and `role_grants` row counts are unchanged, keyed on each tool's audit event; the project MCP server's handler set contains no role or grant verb. (§7.3 B) **(v1.2)** The same test proves a manager's live `workspace.create_bot` call parks pending approval with `roles` and `role_grants` unchanged, and after approval creates exactly one role holding only floor grants; `workspace.retire_bot` likewise parks and changes nothing until approved.
