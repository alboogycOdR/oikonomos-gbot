...
SOURCE DOCUMENTS — ingest in this order. Precedence is highest-first; where they conflict, the higher document wins:
  ...
  2c. docs/architecture/OIKONOMOS_WBS_Addendum_B_v1.0.md — OpenSandbox adoption.
      SUPERSEDES Epic E5 in full (see Addendum B §2 for the rewritten ticket set;
      OIK-046 is RETIRED, do not schedule it). Narrows E8 tickets OIK-076/077 and
      narrows OIK-121 (Addendum B §3). Adds tickets OIK-045a/045b/045c.
  2d. docs/architecture/OIKONOMOS_WBS_Addendum_C_v1.0.md — DeepSeek Harness (dsh)
      findings. Adds tickets OIK-159..162 to E4 only. No epic superseded, no gate
      affected. Low-weight, non-blocking.
  2e. docs/architecture/OIKONOMOS_WBS_Addendum_D_v1.0.md — OpenMausBot findings.
      Adds tickets OIK-163..167 (harvest/evaluation/watch only — no adoption).
      Amends OIK-097 and informs OIK-149/E9. No epic superseded, no gate affected.
...
- E5 is now the OpenSandbox integration (Addendum B), not hand-built Docker isolation. Sequence OIK-042/043 early and in parallel with E2, so E3/E4 land directly on the isolated runtime rather than being retrofitted onto it later. OIK-046 no longer exists as a schedulable item.
- OIK-121 (OpenBao) is narrowed by Addendum B to browser-session OAuth leasing only — general credential injection into agent sandboxes is covered by OIK-045a instead.
