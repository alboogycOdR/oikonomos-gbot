...
- [2026-09-21T11:30:00Z] [S5] REWORK R1 closed: added DB-backed driver test in chatRunDriver.test.ts (TASK-301 describe) driving real createChatRunDriver on BOTH lanes (claude queryFn systemPrompt; gemini request body): roster member sees project charter + user + own-agent facts + superseding value; never other-agent, superseded, or expired facts; non-member in the SAME project thread never sees charter (would fail if rosterProjectIdFor leaked) but sees user fact. Passes in full test-isolated run (log runS5-306-r3.log). Remaining failures are the pre-existing environmental set (db projects, evals-harness, worker budget.platform_exceeded/timeouts, control-api).
