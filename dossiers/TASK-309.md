...
- [2026-09-24T02:22:00Z] [CX9] Ownership expansion confirmed. Added typed project API clients, a project-thread Work board with a separate attention inbox for blocked items and their server-provided reasons, and a Results artifact register. ChatPage resolves a project solely by its `threadId`; ordinary threads retain the existing routine and receipt views. Preflight evidence: `apps/dashboard/src/components/workspace/project/**` is new territory; WorkView.tsx, ResultsView.tsx, api.ts and api.test.ts exist (192, 200, 631 and 44 lines respectively). Dashboard typecheck and build pass; isolated dashboard suite passes 25 files / 160 tests, including the new project route integration test.
