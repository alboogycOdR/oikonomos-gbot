...
- [2026-08-16T12:00:10Z] [CX] Added the round-three non-blocking fail-closed sentinel regression: a dependency failure followed by an audit-writer failure returns `deny/broker.dependency_failure` with the explicit `unavailable` audit-event sentinel, never an allow or rejection.
