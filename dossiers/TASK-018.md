...
- [2026-08-15T15:42:00Z] [CX] Rework complete: `run-local.mjs` now runs the real protected-path gate against the merge-base with the local integration branch (`master`, then `main`), accepting only the exact Fable marker. Expanded CODEOWNERS and gate coverage to `.claude/**`, `.codex/**`, and `hooks/**`. Verified unmarked protected branch exits 1; `node infra/ci/run-local.mjs --approval-marker fable-reviewed` passes all 10 jobs; protected-path self-test 8/8 passes.
