...
- [2026-09-05T12:33:45Z] [CX] REWORK complete. Confirmed the chat regression was caused by the global FIFO fake-response queue consuming the new handoff requests before existing transcript/SSE responses. `_loadHandoffs` now fetches role details only after a non-empty handoff response, and `FakeHttpClient` supports method/path-specific responses plus an explicit no-handoffs fallback for optional timeline decoration. The handoff test scripts both new endpoints by exact path; the remaining chat tests retain their original request scripts. Verification: `flutter test test/screens/chat_screen_test.dart` 16/16; `flutter analyze` clean; `flutter test` 74/74; `pnpm -r test` exit 0; `pnpm -r build` exit 0; `pnpm lint` exit 0.
