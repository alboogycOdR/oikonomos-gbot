...
- [2026-08-15T06:31:22Z] [CX] Rework complete: set `coverage.enabled: true` in the package Vitest configuration so ordinary `vitest run`, workspace `pnpm test`, and CI all enforce the branch threshold. Reproducible verification passed: `pnpm --filter @oikonomos/policy test` ran 9/9 and reported 100% statements, branches, functions, and lines; `pnpm --filter @oikonomos/policy typecheck`, `build`, `pnpm lint`, workspace `pnpm test`, and `git diff --check` all exited 0. The workspace run showed policy coverage enabled and 100% branches; the two DATABASE_URL-dependent DB tests were skipped as designed.
