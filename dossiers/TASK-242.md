...
- [2026-09-12T14:08:00Z] [CX9] Committed implementation as `c6d9a48 feat(receipts): add tenant-scoped run completion receipt [TASK-242]`. `scripts/test-isolated.ps1 -Init` refreshed only the disposable `oikonomos_test` database. The package-wide isolated DB suite exposed existing unrelated failures in `database.test.ts` (registered-capability count mismatch) and `roles.test.ts` (migration backfill deadlock); TASK-242's three receipt tests pass against that same isolated DB. Full serial workspace invocation was also started with `scripts/test-isolated.ps1`.
