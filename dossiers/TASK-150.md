...
- [2026-09-04T22:09:00Z] [CX9] Post-repair verification (Node v22.23.2, pnpm 10.23.0): inspected the installed `packages/harness-factory/node_modules/@anthropic-ai/claude-agent-sdk/sdk.d.ts`, package v0.3.233 (Claude Code 2.1.233). `SDKResultSuccess` and `SDKResultError` are both terminal `{ type: "result" }` stream messages. Each carries numeric `total_cost_usd`, `usage`, and `modelUsage: Record<string, ModelUsage>`; `ModelUsage` includes `inputTokens`, `outputTokens`, `cacheReadInputTokens`, `cacheCreationInputTokens`, `webSearchRequests`, `costUSD`, `contextWindow`, and `maxOutputTokens`. The SDK declaration explicitly describes `total_cost_usd` and `modelUsage` as cumulative across streaming-input turns, so future composition should apply the tap to the final composed `harness.query` and account for the latest result rather than sum result events. This prototype deliberately leaves composition untouched.

- [2026-09-04T22:10:00Z] [CX9] Fresh required test evidence: `pnpm --filter @oikonomos/harness-factory test` exit 0 — 15 test files / 105 tests passed, including `src/budgetTap.test.ts` (3 tests: unchanged passthrough and pre-yield report, fail-closed sink rejection, cost-less stream uncalled). `pnpm -r test` exit 0. `pnpm -r build` exit 0 across 18 of 19 workspace projects. `pnpm lint` exit 0. No production call site was changed; only the tap implementation, its tests, barrel export, and this dossier are in the task commit.
