...
Your task is $TaskId ($ResumeOrClaim by the dispatcher before this session started - do NOT edit PLAN.md to claim it again; the claim is already recorded for you). This says nothing about git branches: creating your task branch is your job, see step 2.
Procedure: (1) Read AGENTS.md and $Briefing, then PLAN.md, fresh from disk, for $TaskId's Spec_References/Owned_Paths/Acceptance_Criteria AND its **Review_Findings** field. Read dossiers/$TaskId.md in full if it exists (your prior work log) before acting. **REWORK CHECK — do this before concluding you are done: if Review_Findings contains REWORK, the supervisor sent this task back and those findings are your job this session. PLAN.md's Review_Findings OUTRANKS your own dossier: a dossier saying the work is complete with passing tests is stale the moment a REWORK verdict lands, so never treat it as authorisation to re-emit needs_review without addressing the findings.** (2) Branch: if task/$TaskId-$Suffix already exists, check it out and continue at the exact stopping point recorded in the dossier. If it does not exist, CREATE it yourself with git (git checkout -b task/$TaskId-$Suffix) - this is expected and authorised, never a reason to block. Check with git rev-parse --verify rather than assuming. (3) Implement strictly against Spec_References, touching ONLY files under Owned_Paths plus your own dossier (dossiers/$TaskId.md - append a Work Log entry at minimum every ~30 minutes of work and at every stopping point; it is your heartbeat, since you never touch PLAN.md for this). (4) Test everything. (5) Emit a devteam-control block as the LAST thing you print, fenced exactly like this:
...
if ($ControlMode -eq "strict") {
    # DETACHED-LAUNCH RULE (2026-08-27): every builder session, strict or
    # legacy, now runs in its own OS process outside dispatch.ps1's own
    # process tree -- same rationale as the legacy-mode launch below (see its
    # comment), applied here too. Before this fix, strict mode ran the
    # builder CLI BLOCKING inside dispatch.ps1's own process, so a kill of
    # whatever invoked dispatch.ps1 (a harness background-job reap, a
    # terminal close, anything) took a live, healthy builder session down
    # with it before it could commit work or emit its control block. Live
    # failure: three consecutive TASK-058 dispatches on 2026-08-26/27 were
    # killed this way, confirmed by dispatch.ps1's own comment describing the
    # identical class of bug from 2026-08-02. The fix mirrors the legacy
    # runner pattern (prompt-to-file, generated runner script, Start-Process
    # detached) but ALSO performs strict mode's own post-session work --
    # UTF-8 re-encode and devteam-control extraction -- INSIDE the detached
    # runner, so dispatch.ps1 itself never blocks on the builder at all. A
    # ".done" marker file (containing the extractor's output) appears next to
    # the log when the session and extraction are both complete; poll that
    # (or PLAN.md, or /devteam-status) instead of waiting on this process.
    ...
    $DonePath = Join-Path $DevteamDir "$TaskId-$RunTs.done"
    $PromptPath = Join-Path $DevteamDir "$TaskId-$RunTs.prompt.txt"
    $RunnerPath = Join-Path $DevteamDir "$TaskId-$RunTs.run.ps1"

    $Utf8NoBom = New-Object System.Text.UTF8Encoding($false)
    [System.IO.File]::WriteAllText($PromptPath, $Prompt, $Utf8NoBom)

    $RunnerLines = @(
        '# Auto-generated by scripts/dispatch.ps1 (strict mode) -- one builder session, one window.',
    ...
    if ($PromptViaStdin) {
        # Same reasoning as the prior inline version: a multiline prompt
        # cannot survive a cmd /c argument, so it goes in via stdin redirect,
        # with log redirection happening INSIDE cmd (raw UTF-8, no PS
        # streams, no Tee-Object UTF-16, no stderr-to-pipeline exception
        # surface). $CmdLineLiteral is fully resolved HERE, in dispatch.ps1's
        # own process, then embedded as a literal string in the generated
        # runner -- the runner does not need to re-derive $CmdArgs itself.
        $NativeArgs = @($CmdArgs | Select-Object -Skip 1)  # drop leading /c
        $CmdLineLiteral = ($NativeArgs -join " ") + ' - < "' + $PromptPath + '" > "' + $LogPath + '" 2>&1'
        $CmdLineEscaped = $CmdLineLiteral -replace "'", "''"
        $RunnerLines += "& cmd /c '$CmdLineEscaped'"
    } else {
        # Same invocation shape as the legacy runner below: read the prompt
        # from file (BOM/newline-safe) and escape embedded quotes for PS
        # 5.1's argv-splitting defect (see the $PromptArg comment above).
        $RunnerLines += "`$Prompt = [System.IO.File]::ReadAllText('$PromptPath')"
        $RunnerLines += "`$Prompt = `$Prompt -replace '`"', '\`"'"
        $RunnerLines += "& '$Cmd' (@('$($CmdArgs -join "','")') + @(`$Prompt)) 2>&1 | Tee-Object -FilePath '$LogPath'"
    }

    $RunnerLines += @(
        # Re-encode to UTF-8 (no BOM) before extraction: PS 5.1's Tee-Object
        # writes UTF-16 LE, and control.py reads utf-8 -- the fence regex can
        # never match interleaved NULs (live failure 2026-08-14, oikonomos:
        ...
        "}",
        "Write-Host '[$Id] session ended. Extracting devteam-control block...' -ForegroundColor Cyan",
        "`$ExtractOut = (& '$Py' 'scripts\control.py' 'extract' '--log' '$LogPath' '--task' '$TaskId' '--unit' '$Id' '--repo' '$RepoRoot' | Out-String).Trim()",
        "Write-Host ('[$Id] ' + `$ExtractOut)",
        "if (`$ExtractOut -like 'UNREPORTED:*') {",
        "    Write-Warning '[$Id] NOTE: no CONTROL block found - PLAN.md state will not change until the next supervisor tick fallback. Log: $LogPath'",
        "}",
        "Set-Content -LiteralPath '$DonePath' -Value `$ExtractOut -Encoding utf8",
        "Write-Host ''",
        "Write-Host '[$Id] $TaskId done (control.mode=strict: PLAN.md is applied by the supervisor next tick). Run /devteam-status in the ORCH session.' -ForegroundColor Green",
    ...
    Write-Host "[dispatch] $Id launched in its OWN window - detached from this process tree ($ResumeOrClaim $TaskId)." -ForegroundColor Green
    Write-Host "[dispatch]   transcript:  $LogPath"
    Write-Host "[dispatch]   done marker: $DonePath  (appears once the session ends and the control block is extracted)"
    Write-Host "[dispatch]   runner:      $RunnerPath"
    Write-Host "[dispatch] This script does NOT wait. Poll the done marker, PLAN.md, or run /devteam-status." -ForegroundColor Cyan
    exit 0
