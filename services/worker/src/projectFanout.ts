...
export function createProjectFanoutAdmission(identity: ProjectToolIdentity): (input: Omit<ProjectFanoutAdmissionInput, "tenantId" | "runId" | "actor">) => Promise<ProjectFanoutAdmission> {
  return (input) => {
    ...
    return admitProjectFanout(
      ...
      { ...input, tenantId: identity.tenantId, runId: identity.runId, actor: `agent:${identity.fromRoleId}` },
    );
  };
}
